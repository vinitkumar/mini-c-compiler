...
class x86Registers:
    ...
    def save_callee_saves(self):
        ...
        for reg in self.callee_save_regs_used:
            self.o("  pushl %s" % reg, "Save callee-save register")
    ...
    def load_callee_saves(self):
        ...
        for reg in self.callee_save_regs_used:
            self.o("  popl %s" % reg, "Restore callee-save register")
    ...
    def _copy_reg_to_temp(self, valid_regs, comment_str=None):
        ...
        self.o("  movl %s, %s" % (reg, mem), comment_str)
    ...
    def _get_free_reg(self, valid_regs, preferred_reg=None):
        ...
        if len(self.regs_free) > 0:
            ...
            if reg != None:
                ...
                if (
                    reg in self.callee_save_regs
                    and reg not in self.callee_save_regs_used
                ):
    ...
    def _coerce_type(self, curr_reg, from_type, to_type):
        ...
        if from_str == "char":
            ...
            if to_str == "char":
                self.o("  movzbl %s, %s" % (self.lo(curr_reg), curr_reg), comment_str)
                return curr_reg
    ...
    def _pop(self, valid_regs):
        ...
        self.o("  movl %s, %s" % (loc, reg), "Stack machine: copy temp to register")
...
class CodeGenVisitor(Visitor):
    ...
    def _calc_function_arg_addrs(self, symtab):
        ...
        for symbol in list(symtab.entries.values()):
            symbol.compile_loc = "%d(%%ebp)" % (
                WORD_SIZE * 2 + (symbol.param_num * WORD_SIZE)
            )
    ...
    def vFunctionDefn(self, node):
        ...
        if self.stack.get_max_fp() != 0:
            self.o(
                "  subl $%d, %%esp" % (-self.stack.get_max_fp()),
    ...
    def vIfStatement(self, node):
        ...
        self.o("  jz %s" % else_label, "If result is zero, jump to else clause")
    ...
    def vWhileLoop(self, node):
        ...
        self._push_loop_labels(break_label=done_label, continue_label=test_label)
        ...
        self.stack.done()
        self.o("  testl %s, %s" % (comparer, comparer), "Test the result")
        self.o("  jz %s" % done_label, "If result is zero, leave while loop")
    ...
    def vForLoop(self, node):
        ...
        self.stack.done()
        self.o("  testl %s, %s" % (comparer, comparer), "Test the result")
        self.o("  jz %s" % done_label, "If result is zero, leave for loop")
        self._accept_and_empty_stack(node.stmt)
    ...
    def vBreakStatement(self, node):
        self.o("  jmp %s" % self.break_labels[-1], "Loop: break statement")

    def vContinueStatement(self, node):
        self.o("  jmp %s" % self.continue_labels[-1], "Loop: continue statement")
    ...
    def vStringLiteral(self, node):
        ...
        self.o(
            "  movl $%s, %s" % (label_str, self.stack.push(node.type)),
            "Get addr of string literal '%s'" % comment_label,
        )

    def vConst(self, node):
        self.o(
            "  movl $%d, %s" % (node.value, self.stack.push(node.type)),
    ...
    def vId(self, node):
        ...
        if node.output_addr:
            self.o(
                "  leal %s, %s" % (node.symbol.compile_loc, self.stack.push()),
        ...
        self.o(
            "  %s %s, %s"
            % (instr, node.symbol.compile_loc, self.stack.push(node.type)),
    ...
    def vArrayExpression(self, node):
        ...
        if node.output_addr:
            self.o(
                "  leal %s, %s" % (addr_str, reg_to), "Load addr of pointer array index"
            )
        else:
            type_str = node.type.get_outer_string()
    ...
    def vFunctionExpression(self, node):
        ...
        self.c("FUNCTION CALL to %s() - begin" % node.function.symbol.name)
        ...
        self.o(
            "  call %s" % node.function.symbol.compile_loc,
        ...
        result = self.stack.push(
            node.function.symbol.type.get_return_type(), preferred_reg="%eax"
        )
        ...
        if arg_stack_size > 0:
            self.o("  addl $%d, %%esp" % arg_stack_size, "Deallocate argument stack")
    ...
    def _binop_assign(self, node):
        ...
        self.o(
            "  %s %s, (%s)" % (instr, right_reg, left_reg),
        ...
        self.o(
            "  %s (%s), %s" % (instr, left_reg, self.stack.push(node.type)),
    ...
    def _binop_arith(self, node):
        ...
        instr = self.binop_instructions[node.op] + self._type_suffix(node.type)
        type_str = node.type.get_outer_string()
        ...
        self.o("  %s %s, %s" % (instr, r_reg, l_reg), "Perform '%s'" % node.op)
        self.stack.done()
    ...
    def _binop_compare(self, node):
        ...
        self.o(
            "  %s %s" % (self.binop_instructions[node.op], lo), "Perform '%s'" % node.op
        )
        self.o("  movzbl %s, %s" % (lo, byte_reg), "Zero-extend the boolean result")
    ...
    def vNegative(self, node):
        node.expr.accept(self)
        self.o("  negl %s" % self.stack.peek(), "Perform unary negation")
    ...
    def vPointer(self, node):
        ...
        if type_str in ["int", "pointer"]:
            ...
            instr = "movzbl"
        self.o("  %s (%s), %s" % (instr, reg_from, reg_to), "Pointer dereference")
        self.stack.done()
