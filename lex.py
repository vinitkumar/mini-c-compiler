...
#
# $Header: /cygdrive/c/prog/CVS/mini_c/lex.py,v 1.1.1.1 2004/05/27 06:40:38 varmaa Exp $
#
    ...
    for f in fsymbols:
        ...
        if f.__doc__:
            if not optimize:
                ...
                if debug:
                    print("lex: Adding rule {} -> '{}'".format(f.__name__,f.__doc__))

            # Okay. The regular expression seemed okay.  Let's append it to the master regular
            # expression we're building
  
            if (regex): regex += "|"
            regex += "(?P<{}>{})".format(f.__name__,f.__doc__)
        else:
    ...
    for name,r in ssymbols:
        ...
        if not optimize:
            ...
            if name[2:] not in lexer.lextokens:
                print("lex: Rule '{}' defined for an unspecified token {}.".format(name,name[2:]))
                error = 1
                continue
            try:
                ...
                continue
            if debug:
                print("lex: Adding rule {} -> '{}'".format(name,r))
                
        if regex: regex += "|"
        regex += "(?P<{}>{})".format(name,r)
